...
public final class MagnetItem extends Item {
    ...
    private static final String RANGE_KEY = "advancedtools_magnet_range";
    private static final String ENABLED_KEY = "advancedtools_magnet_enabled";
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (!level.isClientSide) {
            int size = next * 2 + 1;
            player.displayClientMessage(Component.translatable("message.advancedtools.magnet.range", size, size), true);
        }
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltip, TooltipFlag flag) {
        ...
        tooltip.add(Component.translatable("tooltip.advancedtools.magnet.range", size, size).withStyle(ChatFormatting.GRAY));
        tooltip.add(Component.translatable(isEnabled(stack)
                ? "tooltip.advancedtools.magnet.enabled"
                : "tooltip.advancedtools.magnet.disabled")
                .withStyle(isEnabled(stack) ? ChatFormatting.GREEN : ChatFormatting.RED));
        tooltip.add(Component.translatable("tooltip.advancedtools.magnet.toggle").withStyle(ChatFormatting.DARK_GRAY));
    }
